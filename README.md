...
The website at **https://xnk.my.id**. It is a GitHub Pages site that shows the XNK
booking app (a Google Apps Script web app) full-screen, with a loading screen,
link previews for WhatsApp and social media, and "Add to Home Screen" support.
...
| File | What it is |
| --- | --- |
| `index.html` | The page. Shows the booking app in an iframe, plus the loading screen. |
...
| `og-image.png` | Preview image shown when the link is shared (1200 × 630). |
...
`index.html`. The "Buka di tab baru" link reads it from there. Redeploying the
...
**Description text.** `Pesan jadwal dengan mudah secara online.` appears in
`index.html` (`description` and `og:description`) and in `manifest.webmanifest`.
...
| `og-image.png` | 1200 × 630 | `#111111` | logo + "XNK" in the center |

## After publishing

- In **Settings → Pages**, make sure **Enforce HTTPS** is ticked.
- WhatsApp and Facebook cache link previews. After changing the preview text or
  image, paste the link into the
  [Facebook Sharing Debugger](https://developers.facebook.com/tools/debug/) and
  press **Scrape Again**; WhatsApp follows within a day or so.
